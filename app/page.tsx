...
export default function Home() {
  ...
  function makeCanvas(z=8888) {
    ...
    c.style.cssText = `position:fixed;inset:0;z-index:${z};pointer-events:none;width:100%;height:100%`;
  ...
  function runCommand(cmd: string, durationSec?: number) {
    ...
    if (cmd === "earthquake") {
      ...
      tick();effectsRef.current.push({stop:()=>{stopped2=true;cancelAnimationFrame(raf2);root.style.transform=orig;}});autoStop((durationSec??10)*1000);return;
    }
    if (cmd === "blackout") {
      const c=makeCanvas(99999); const ctx2=c.getContext("2d")!;
      ctx2.fillStyle="#000";ctx2.fillRect(0,0,c.width,c.height);
      effectsRef.current.push({stop:()=>c.remove()});autoStop((durationSec??10)*1000);return;
    }
    if (cmd === "strobe") {
    ...
    if (cmd === "oldtv") {
      ...
      tick();effectsRef.current.push({stop:()=>{stopped2=true;cancelAnimationFrame(raf2);c.remove();}});autoStop((durationSec??4)*1000);return;
    }
    if (cmd === "windows") {
      const d=document.createElement("div");
      ...
      document.body.appendChild(d);
      effectsRef.current.push({stop:()=>d.remove()});autoStop((durationSec??15)*1000);return;
    }
    if (cmd === "bsod") {
      const c=makeCanvas(99999); const ctx2=c.getContext("2d")!;
      ...
      effectsRef.current.push({stop:()=>c.remove()});autoStop((durationSec??15)*1000);return;
    }
    if (cmd === "loading") {
      const d=document.createElement("div");
    ...
    if (cmd === "zoom") {
      ...
      autoStop((durationSec??8)*1000); return;
    }
    ...
    if (cmd === "rage") {
      ...
      autoStop((durationSec??6)*1000); return;
    }
  ...
  useEffect(() => {
    ...
    const unsubAuth = onAuthStateChanged(auth, (u) => {
      ...
      const startedEffectKeys = new Set<string>();
      onValue(ref(db, "config/activeEffects"), (snap) => {
        ...
        Object.entries(effects).forEach(([key, { cmd, durationSec, startedAt }]) => {
          if (startedEffectKeys.has(key)) return; // already running this one
          const elapsed = (Date.now() - startedAt) / 1000;
          const remaining = Math.floor(durationSec - elapsed);
          if (remaining > 2) { startedEffectKeys.add(key); runCommand(cmd, remaining); }
        });
      });
      ...
      onValue(ref(db, `users/${u.uid}/pendingCmd`), (snap) => {
        ...
        if (cmd) runCommand(cmd, duration || undefined);
      });
    });
  ...
  useEffect(() => { isAdminRef.current = !!userData?.isAdmin; }, [userData]);
...
{ version:"v1.9.0 — Friday, June 6, 2026", date:"Friday, June 6, 2026", items:[
  ...
  const AuthHeader = () => (
    <>
      ...
      <button onClick={() => setShowLangModal(true)}
        style={{ position:"fixed", top: announcement ? (isMobile ? 44 : 50) : 12, left:16, zIndex:200, transition:"top 0.2s", display: screen === "game" ? "none" : undefined, background:"rgba(15,15,26,0.7)", border:"1px solid rgba(45,45,68,0.6)", borderRadius:8, color:"#9ca3af", fontSize:12, fontWeight:600, padding:"5px 12px", cursor:"pointer" }}>
      ...
      <div style={{ position:"fixed", top: announcement ? (isMobile ? 44 : 38) : 0, right:0, padding:"12px 16px", zIndex:200, display: screen === "game" ? "none" : "flex", alignItems:"center", gap:10, transition:"top 0.2s" }}>
