...
const QUESTIONS = [
  ...
  { q: "What gas do plants absorb from the air?", a: "CO₂", wrong: ["O₂", "N₂", "H₂"] },
  ...
  { q: "What is the capital of Brazil?", a: "Brasília", wrong: ["São Paulo", "Rio de Janeiro", "Salvador"] },
...
export default function Home() {
  ...
  const [leaderboard, setLeaderboard] = useState<{name:string,score:number,streak:number,date:string}[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      const n = localStorage.getItem("onetap_name") || "";
      setName(n);
    } catch {}
  }, []);
  ...
  const nextQuestion = useCallback((qs: typeof QUESTIONS, idx: number) => {
    if (idx >= qs.length) {
      endGame(score, bestStreak, correct, total);
      return;
    }
    setQIndex(idx);
    setOptions(getOptions(qs[idx]));
    setSelected(null);
    setTimeLeft(3);
    setAnim("");
    answerRef.current = false;
  }, [score, bestStreak, correct, total, endGame]);

  const handleAnswer = useCallback((ans: string, qs: typeof QUESTIONS, idx: number, currentStreak: number, currentScore: number, currentCorrect: number, currentTotal: number) => {
    ...
    const newScore = isCorrect ? currentScore + 10 + bonus : currentScore;
    ...
    setTimeout(() => {
      if (idx + 1 >= qs.length) { endGame(newScore, Math.max(newStreak, bestStreak), newCorrect, newTotal); }
      else nextQuestion(qs, idx + 1);
    }, 800);
  }, [bestStreak, endGame, nextQuestion]);
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          ...
          handleAnswer("__timeout__", questions, qIndex, streak, score, correct, total);
          return 0;
        }
        return t - 1;
      });
  ...
  }, [screen, qIndex, selected, questions, streak, score, correct, total, handleAnswer]);

  function startGame() {
    const qs = shuffle(QUESTIONS).slice(0, 15);
    setQuestions(qs);
    setQIndex(0);
    setOptions(getOptions(qs[0]));
    setSelected(null);
    setTimeLeft(3);
    setStreak(0);
    setBestStreak(0);
  ...
  function saveName(n: string) {
    setName(n);
    try { localStorage.setItem("onetap_name", n); } catch {}
  }
  ...
  if (screen === "home") return (
    <div style={{ minHeight: "100vh", background: "#0f0f1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px", color: "#fff" }}>
      ...
      <div style={{ width: "100%", maxWidth: 400, background: "#1a1a2e", borderRadius: 16, padding: "24px", marginBottom: 20 }}>
        ...
        <input value={name} onChange={e => saveName(e.target.value)} placeholder="Enter your name..."
      ...
      <button onClick={startGame}
        style={{ background: "linear-gradient(135deg, #f59e0b, #ef4444)", border: "none", borderRadius: 14, color: "#fff", fontSize: "1.2rem", fontWeight: 800, padding: "18px 48px", cursor: "pointer", marginBottom: 32, letterSpacing: "-0.01em" }}>
        START GAME ⚡
      </button>
      ...
      {leaderboard.length > 0 && (
        <div style={{ width: "100%", maxWidth: 400, background: "#1a1a2e", borderRadius: 16, padding: "20px" }}>
          ...
          {leaderboard.slice(0, 5).map((e, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < 4 ? "1px solid #2d2d44" : "none" }}>
  ...
  if (screen === "result") return (
    <div style={{ minHeight: "100vh", background: "#0f0f1a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "20px", color: "#fff" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 32, width: "100%", maxWidth: 400 }}>
        {[["Score", score, "#f59e0b"], ["Best Streak", bestStreak + "🔥", "#ef4444"], ["Accuracy", Math.round((correct/total)*100) + "%", "#10b981"]].map(([label, val, color]) => (
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
        <button onClick={startGame}
          style={{ background: "linear-gradient(135deg, #f59e0b, #ef4444)", border: "none", borderRadius: 12, color: "#fff", fontSize: "1rem", fontWeight: 800, padding: "14px 28px", cursor: "pointer" }}>
          PLAY AGAIN ⚡
        </button>
        <button onClick={() => setScreen("home")}
          style={{ background: "#1a1a2e", border: "1px solid #2d2d44", borderRadius: 12, color: "#9ca3af", fontSize: "1rem", fontWeight: 600, padding: "14px 28px", cursor: "pointer" }}>
          Home
        </button>
      </div>
      ...
      {leaderboard.length > 0 && (
        <div style={{ width: "100%", maxWidth: 400, background: "#1a1a2e", borderRadius: 16, padding: "20px" }}>
          <div style={{ fontSize: 13, color: "#f59e0b", marginBottom: 14, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 700 }}>🏆 Leaderboard</div>
          {leaderboard.slice(0, 5).map((e, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < 4 ? "1px solid #2d2d44" : "none", background: e.name === (name||"Anonymous") && e.score === score ? "rgba(245,158,11,0.05)" : "transparent", borderRadius: 6, padding: "8px 6px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 18, width: 24 }}>{["🥇","🥈","🥉","4️⃣","5️⃣"][i]}</span>
                <span style={{ color: "#e5e7eb", fontWeight: 600 }}>{e.name}</span>
              </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0f1a", display: "flex", flexDirection: "column", alignItems: "center", padding: "20px", color: "#fff" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 16, fontWeight: 700, color: streak > 0 ? "#ef4444" : "#4b5563" }}>
          🔥{streak}
        </div>
      </div>
      ...
      <div style={{ position: "relative", width: 80, height: 80, marginBottom: 24 }}>
        <svg width="80" height="80" style={{ transform: "rotate(-90deg)" }}>
          ...
          <circle cx="40" cy="40" r="34" fill="none"
            ...
            strokeDashoffset={213.6 * (1 - timeLeft / 3)}
            style={{ transition: "stroke-dashoffset 0.9s linear, stroke 0.3s" }} />
        </svg>
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 900, color: timeLeft <= 1 ? "#ef4444" : "#fff" }}>{selected ? "✓" : timeLeft}</div>
      </div>

      {/* Streak popup */}
      {showStreak && (
        <div style={{ position: "fixed", top: "30%", left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg, #f59e0b, #ef4444)", borderRadius: 16, padding: "12px 24px", fontSize: 22, fontWeight: 900, zIndex: 100, animation: "pop 0.3s ease" }}>
          🔥 {streak}x STREAK!
        </div>
      )}
      ...
      <div style={{ width: "100%", maxWidth: 480, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {options.map((opt, i) => {
          ...
          let bg = "#1a1a2e";
          let border = "2px solid #2d2d44";
          let color = "#e5e7eb";
          if (showResult && isCorrect) { bg = "#064e3b"; border = "2px solid #10b981"; color = "#10b981"; }
          if (showResult && isWrong) { bg = "#450a0a"; border = "2px solid #ef4444"; color = "#ef4444"; }
          return (
            <button key={i}
              onClick={() => handleAnswer(opt, questions, qIndex, streak, score, correct, total)}
              disabled={!!selected}
              className={isSelected ? anim : ""}
              style={{ background: bg, border, borderRadius: 14, color, fontSize: "1rem", fontWeight: 700, padding: "20px 16px", cursor: selected ? "default" : "pointer", transition: "all 0.2s", textAlign: "center", lineHeight: 1.3 }}>
              {opt}
            </button>
          );
        })}
